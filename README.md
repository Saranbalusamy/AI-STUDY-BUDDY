# StudyBuddy 📚🔍
An AI-powered chat interface for querying PDF documents. Built using Langchain, Groq (Llama 3.3), Pinecone, and NextJS 13.
...
1. A [Groq](https://console.groq.com/) API key
...
4. Create a ```.env``` file and populate it with your `GROQ_API_KEY` and `PINECONE_API_KEY` variables.
...
> **Note:** Embeddings are generated locally using `Xenova/all-MiniLM-L6-v2`. The first run may take a moment while the model downloads.
